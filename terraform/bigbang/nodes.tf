...
resource "azurerm_linux_virtual_machine" "rancher_server" {
  ...
  computer_name         = "${local.computer_name_prefix}-bb-dsop" // ensure computer_name meets 15 character limit
...
resource "azurerm_linux_virtual_machine" "workers" {
  ...
  computer_name         = "${local.computer_name_prefix}-bb-dsop-worker-${count.index}" // ensure computer_name meets 15 character limit
  location              = data.azurerm_resource_group.rancher_demo.location
  resource_group_name   = data.azurerm_resource_group.rancher_demo.name
  ...
  custom_data = data.cloudinit_config.worker_config.rendered
